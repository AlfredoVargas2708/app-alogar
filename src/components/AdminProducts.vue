...
async function onSave({ id, payload }: { id: number | null; payload: ProductPayload }) {
    ...
    try {
        if (id !== null) {
            ...
            toast.add({ severity: 'success', summary: 'Producto Actualizado', detail: 'Los cambios se guardaron correctamente.', life: 4000 });
        } else {
            await createProduct(payload);
            toast.add({ severity: 'success', summary: 'Producto Creado', detail: 'El producto se creó correctamente.', life: 4000 });
        }
        dialogVisible.value = false;
        loadProducts(Math.floor(first.value / rows.value) + 1);
    } catch (err: unknown) {
        toast.add({ severity: 'error', summary: 'Error al Guardar', detail: apiError(err, 'No se pudo guardar el producto.'), life: 5000 });
    } finally {
        saving.value = false;
    }
}
    ...
    confirm.require({
        ...
        accept: async () => {
            try {
                ...
                toast.add({ severity: 'success', summary: 'Producto Eliminado', detail: `"${product.title}" fue eliminado.`, life: 4000 });
                loadProducts(Math.floor(first.value / rows.value) + 1);
            } catch (err: unknown) {
                toast.add({ severity: 'error', summary: 'Error al Eliminar', detail: apiError(err, 'No se pudo eliminar el producto.'), life: 5000 });
            }
        },
    });
}
...
async function onToggleAvailable(product: Product, available: boolean) {
    ...
    try {
        ...
        toast.add({ severity: 'success', summary: 'Disponibilidad', detail: `"${product.title}" ahora está ${available ? 'disponible' : 'agotado'}.`, life: 3000 });
    } catch (err: unknown) {
        toast.add({ severity: 'error', summary: 'Error', detail: apiError(err, 'No se pudo actualizar la disponibilidad.'), life: 5000 });
    } finally {
    ...
    <div class="admin-products">
        <DataTable :value="adminProducts" :loading="adminLoading" data-key="id" lazy paginator :first="first"
            ...
            <Column header="Oferta" style="width: 130px">
                <template #body="{ data }">
                    <span v-if="data.offer_price" class="offer-price">{{ formatCurrency(data.offer_price) }}</span>
                    <span v-else class="empty-message">—</span>
                </template>
            </Column>
            <Column header="Disponible" style="width: 110px">
                <template #body="{ data }">
                    <ToggleSwitch :model-value="data.available" :disabled="togglingId === data.id"
            ...
            <Column header="Acciones" style="width: 130px">
                <template #body="{ data }">
                    <div class="row-actions">
                        <Button class="edit-button" rounded :aria-label="`Editar ${data.title}`" @click="openEdit(data)">
                            <Pencil :size="16" />
                        </Button>
                        <Button severity="danger" rounded :aria-label="`Eliminar ${data.title}`"
                            @click="onDelete(data)">
                            <Trash :size="16" />
                        </Button>
                    </div>
        ...
        <ProductFormDialog v-model:visible="dialogVisible" :product="editingProduct" :saving="saving"
            @save="onSave" />
    </div>
</template>
